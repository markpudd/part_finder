...
.config(['$routeProvider', function($routeProvider) {
    $routeProvider.when('/stores.html', {
        ...
        templateUrl: 'partials/store_load.html',
        controller: 'StoreLoadController'
    }).when('/store_section.html/:pid/:sid', {
        templateUrl: 'partials/store_section.html',
        controller: 'StoreSectionController'
    }).when('/new_store.html/:pid', {
        templateUrl: 'partials/new_store.html',
        controller: 'StoreController'            
    }).when('/store_show.html/:pid', {
            templateUrl: 'partials/store_show.html',
            controller: 'StoreLoadController'
        });
}]);
...
partControllers.controller('StoreController', ['$scope', '$location','$http', 'Configs', 'Stores', function($scope, $location,$http, Configs,Stores) {
  ...
  return function (items) {
    ...
    if(items) {
        for (var i = 0; i < items.length; i++) {
          var item = items[i];
          if (item['sub_parts'].length > 0  && !item['sub_parts'][0]['location']) {
            filtered.push(item);
          }
        }
    } 
...
storeControllers.controller('StoreLoadController', ['$scope', '$http', '$routeParams','Parts','Stores','Configs', function($scope, $http,$routeParams,Parts,Stores,Configs) {
    ...
    $scope.categories =  Parts.categories( function() {
          ...
          console.log("Setting part")
          $scope.part_list = Parts.query({category:$scope.categorySelected});
    });
    ...
    $scope.drop = function(partId,destId) {
        ...
        Parts.place({'part_id':partId,'store_id':$scope.storeSelected['_id'],'place_id':destId} , function() {
            ...
            for (var i = 0; i < $scope.part_list.length; i++) {
                if ($scope.part_list[i]['_id'] == partId) {          
                    var loc =  { 'store_id':$scope.storeSelected['_id'],'place_id':destId};
                    $scope.part_list[i]['sub_parts'][0]['location'] = loc;
                }
            }
            // Add part to drawer
